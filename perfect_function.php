...
function _verify_hash($plain_text_str, $hashed_string)
{
	...
	// Not hashed password: Your choice if you want to hash it... - DAN MAZO
	if ($plain_text_str == $hashed_string) {
		return true;
	} else {
		return false;
	}
...
function _get_username_from_id( $id) {
	$user_data = get_where("manipulator", $id);
	foreach ($user_data as $key => $row) {
		return $row['username'];
	}
}

function _get_acct_type_from_id( $id) {
	$user_data = get_where("manipulator", $id);
	foreach ($user_data as $key => $row) {
...
{
	$result = get_where_custom("manipulator", "username", $username);
	foreach ($result as $key => $row) {
		return $row['id'];
	}
}
...
function get_img_from_id($id)
{
	$user_data = get_where("manipulator", $id);
	foreach ($user_data as $key => $row) {
		return $acct_img = $row['photo'];
	}
}
